...
func _ready():
	randomize()
	$NewSlime.wait_time = randf_range(6,10)
	$NewSlime.start()
	$NewNecromancer.wait_time = randf_range(10,15)
	$NewNecromancer.start()
	$NewWine.wait_time = randf_range(4,7)
	$NewWine.start()
	$NewLocker.wait_time = randf_range(6, 10)
	$NewLocker.start()

func _on_new_slime_timeout():
	if _count_slime < randi_range(3,7):
		var _enemy = _new_slime.instantiate()
		add_child(_enemy)
		_enemy.position.x = randf_range(0, 695)
		_enemy.position.y = randf_range(0, 400)
		_count_slime +=1
...
func _on_new_wine_timeout():
	if _count_wine < randi_range(3,7):
		var _wine = _new_wine.instantiate()
		add_child(_wine)
		_wine.position.x = randf_range(0, 695)
		_wine.position.y = randf_range(0, 400)
		_count_wine +=1
