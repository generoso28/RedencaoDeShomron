...
func _ready():
	...
	randomize()
	$NewSlime.wait_time = randf_range(2, 4)
	$NewSlime.start()
	$NewNecromancer.wait_time = randf_range(5, 8)
	$NewNecromancer.start()
	$NewWine.wait_time = randf_range(7,10)
	$NewWine.start()
	$NewLocker.wait_time = randf_range(6, 10)
	$NewLocker.start()

func _on_new_slime_timeout():
	if _count_slime < randi_range(4,7):
		var _enemy = _new_slime.instantiate()
		add_child(_enemy)
		_enemy.position.x = randf_range(-110, 799)
		_enemy.position.y = randf_range(-110, 478)
		_count_slime +=1
...
func _on_new_wine_timeout():
	if _count_wine < randi_range(3,5):
		var _wine = _new_wine.instantiate()
		add_child(_wine)
		_wine.position.x = randf_range(-110, 799)
		_wine.position.y = randf_range(-110, 478)
		_count_wine +=1
	pass # Replace with function body.


func _on_new_locker_timeout():
	if _count_locker < randi_range(4,6):
		var _locker = _new_locker.instantiate()
		add_child(_locker)
		_locker.position.x = randf_range(-110, 799)
		_locker.position.y = randf_range(-110, 478)
		_count_locker +=1
